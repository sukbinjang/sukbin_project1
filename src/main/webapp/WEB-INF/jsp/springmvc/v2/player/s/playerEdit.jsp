...
<main>
    <h3>개인 정보 수정</h3>
    <form action="./app/springmvc/v2/player/s/updatePlayer" method="post">
        <p><input type="email" name="email"
                  value="${empty player.email? sessionScope.PLAYER.email : player.email}"
                  placeholder="이메일" required autofocus/>
        </p>
        <p><input type="name" name="name"
                  value="${empty player.name? sessionScope.PLAYER.name : player.name}"
