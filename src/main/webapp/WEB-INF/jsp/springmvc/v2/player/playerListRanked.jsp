...
<%@ include file="/WEB-INF/jsp/springmvc/v2/rankedListNav.jsp" %>

<main>
    <h3><%=tier%>티어 플레이어 목록</h3>
    <p>총 ${count}건, ${maxPage}페이지</p>
    ...
    <div class="list rankingPlayer">
        <div class="tableColumn">rank</div>
        <div class="tableColumn">ranking point</div>
        <div class="tableColumn">email</div>
        <div class="tableColumn">username</div>
        <c:forEach var="player" items="${rankedPlayerList}" varStatus="index">
            <div>${index.count}</div>
            <div>${player.rankingPoint}</div>
            <div><a
